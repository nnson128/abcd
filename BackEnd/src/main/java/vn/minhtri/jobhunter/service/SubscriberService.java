...
import vn.minhtri.jobhunter.repository.JobRepository;
import vn.minhtri.jobhunter.repository.SkillRepository;
import vn.minhtri.jobhunter.repository.SubscriberRepository;
...
public class SubscriberService {
    ...
    public static String toSlug(String input) {
        String normalized = java.text.Normalizer.normalize(input, java.text.Normalizer.Form.NFD); // tách dấu
        String slug = normalized.replaceAll("[\\p{InCombiningDiacriticalMarks}]", ""); // loại bỏ dấu
        slug = slug.toLowerCase().replaceAll("[^a-z0-9\\s-]", ""); // loại ký tự đặc biệt
        slug = slug.trim().replaceAll("\\s+", "-"); // chuyển khoảng trắng thành -
        return slug;
    }

    public ResEmailJob convertJobToSendEmail(Job job) {
        ResEmailJob res = new ResEmailJob();
        res.setNameSlug(SubscriberService.toSlug(job.getName()));
        res.setIdJob(String.valueOf(job.getId()));
